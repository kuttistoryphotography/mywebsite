...
function PasteButton({
  ...
  const handlePaste = async () => {
    ...
    const fallback = window.prompt(
      "Clipboard access is blocked. Paste your text here and click OK:"
    );
  ...
  return (
    <button
      ...
      className="absolute right-2 top-1/2 -translate-y-1/2 z-10 inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-amber-500 hover:text-black hover:border-amber-500 transition-colors text-xs"
    >
      {pasted ? (
    ...
    fetch("/api/faq")
      ...
      .then((data) => {
        if (data.settings) {
          ...
          if (Array.isArray(data.settings.seoKeywords)) {
            setSeoKeywordsText(data.settings.seoKeywords.join(", "));
          }
        }
      })
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div>
        ...
        <p className="text-sm text-zinc-500 mt-1">
          Control all questions, answers, categories, section
          header, and SEO keywords shown on the FAQ page.
        </p>
      </div>
      ...
      {activeTab === "header" && (
        <div className="bg-zinc-900 rounded-2xl p-6 space-y-5 border border-zinc-800">
          ...
          <p className="text-zinc-500 text-sm -mt-2">
            The label, main heading, and description shown at
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 pr-24 py-2.5 text-white text-sm focus:border-amber-500 focus:outline-none"
              />
          ...
          <div>
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5 pr-24 text-white text-sm focus:border-amber-500 focus:outline-none resize-none"
              />
              ...
              <div className="flex items-center gap-3 px-5 py-4">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 pr-24 py-2 text-white text-sm font-semibold focus:border-amber-500 focus:outline-none"
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <input
                              ...
                              className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 pr-24 py-2 text-white text-sm focus:border-amber-500 focus:outline-none"
                            />
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <textarea
                              ...
                              className="w-full bg-zinc-700 border border-zinc-600 rounded-lg px-3 py-2 pr-24 text-white text-sm focus:border-amber-500 focus:outline-none resize-none"
                            />
      ...
      {activeTab === "seo" && (
        <div className="bg-zinc-900 rounded-2xl p-6 space-y-5 border border-zinc-800">
          <div>
            <h3 className="text-white font-semibold text-lg">
              FAQ Page SEO Keywords
            </h3>

            <p className="text-zinc-500 text-sm mt-1">
              Add relevant search keywords for the FAQ page.
          ...
          <div>
            ...
            <div className="relative">
              <textarea
                ...
                rows={6}
                placeholder="Kutti Story Photography FAQ, wedding photographer FAQ Madurai, wedding photography Madurai, candid photography Madurai"
                onChange={(e) =>
                  setSeoKeywordsText(e.target.value)
                }
                className="w-full bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-3 pr-24 text-white text-sm focus:border-amber-500 focus:outline-none resize-none"
              />

              <PasteButton
                onPaste={(text) =>
                  setSeoKeywordsText(text)
                }
              />
            </div>

            <p className="text-xs text-zinc-600 mt-2">
              {parseKeywords(seoKeywordsText).length}{" "}
              keywords
            </p>
          </div>
          ...
          <div className="bg-zinc-800/50 rounded-xl border border-zinc-700/50 p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {parseKeywords(seoKeywordsText).map(
                (keyword, index) => (
                  <span
                    key={`${keyword}-${index}`}
                    className="px-2.5 py-1 rounded-lg bg-zinc-800 border border-zinc-700 text-xs text-zinc-300"
                  >
                    {keyword}
                  </span>
                )
